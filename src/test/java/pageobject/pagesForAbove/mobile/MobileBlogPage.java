...
public class MobileBlogPage {
    ...
    private final By FB_PAGE_LOGIN_BUTTON = By.xpath(".//input[@value='Log in']");
    private final By PINTEREST_PAGE_SIGN_UP_FORM_TITTLE = By.xpath(".//div[@class='zI7 iyn Hsu']/h1");
    private final By TWITTER_PAGE_SIGN_UP_FORM_TITTLE = By.id("header");
    ...
    public boolean isLinksWorksInRandomBlogItems() {
        LOGGER.info("Checking if randomly selected blog card opens for blog page");
        ...
        if (postExist) {
            ...
            WebElement firstBlogItem = baseFunc.list(BLOG_CONTAINER_ITEMS).get(0);
    ...
    public boolean isMainBlogPostDisplayed() {
        ...
        try {
            baseFunc.waitElementPresented(FB_PAGE_LOGIN_BUTTON);
            if (baseFunc.findElement(FB_PAGE_LOGIN_BUTTON).getAttribute("name").length() > 0) {
                baseFunc.closeTab();
